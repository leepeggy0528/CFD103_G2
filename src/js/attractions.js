$(function () {
    ...
    //新增景點
    $('.joinsite-btn').on('click',function(){
        let target_spot =$(this).prev('a')
        let img_src = target_spot.find('.pic img').attr('src');
        let spot_name = target_spot.find('.spot-name h3').text();
        if(localStorage.getItem('index_spot') != null){
            index_spot = localStorage.getItem('index_spot').split(',');
        }
